...
class TestRemoteTEICrossEncoderInitialization:
    ...
    async def test_initialize_success(self):
        ...
        with patch.object(httpx, "Client", return_value=httpx.Client(transport=transport)):
            ...
            assert encoder._sync_client is not None
...
class TestRemoteTEICrossEncoderPredict:
    ...
    @pytest.fixture
    def initialized_encoder(self):
        """Create an initialized encoder with mocked HTTP client."""
        encoder = RemoteTEICrossEncoder(
            base_url="http://localhost:8080",
            batch_size=128,
            max_concurrent=8,
        ...
        encoder._sync_client = httpx.Client()
    ...
    @pytest.mark.asyncio
    async def test_predict_empty_pairs(self, initialized_encoder):
        """Test predict returns empty list for empty input."""
        result = await initialized_encoder.predict([])
        assert result == []

    @pytest.mark.asyncio
    async def test_predict_single_query(self, initialized_encoder):
        ...
        MockClient = create_mock_async_client(mock_handler)
        ...
        with patch("httpx.AsyncClient", MockClient):
            ...
            scores = await initialized_encoder.predict(pairs)
    ...
    @pytest.mark.asyncio
    async def test_predict_multiple_queries(self, initialized_encoder):
...
class TestRemoteTEICrossEncoderBatching:
    ...
    @pytest.fixture
    def small_batch_encoder(self):
        """Create encoder with small batch size for testing."""
        encoder = RemoteTEICrossEncoder(
            base_url="http://localhost:8080",
    ...
    @pytest.mark.asyncio
    async def test_batch_splitting(self, small_batch_encoder):
        ...
        with patch("httpx.AsyncClient", MockClient):
            ...
            scores = await small_batch_encoder.predict(pairs)
    ...
    @pytest.mark.asyncio
    async def test_score_mapping_across_batches(self, small_batch_encoder):
