...
Hindsight 0.5.0 adds a built-in llama.cpp provider for fully local inference, retain append mode, OpenRouter support, bank templates for portable configuration, a Constellation graph view in the Control Plane, a restructured retain pipeline and capped recall graph expansion for better performance, three new framework integrations (AutoGen, Paperclip, OpenCode), and a breaking removal of legacy graph retrieval strategies and the Hermes integration.

<!-- truncate -->

- [**Built-in llama.cpp Provider**](#built-in-llamacpp-provider): Run Hindsight with fully local LLM inference — no external API calls.
- [**Retain Append Mode**](#retain-append-mode): Concatenate new content onto existing documents with `update_mode='append'`.
- [**OpenRouter Support**](#openrouter-support): Use any OpenRouter model for LLM, embeddings, and reranking.
...
client.retain(bank_id, items=[{"content": "Day 1 notes..."}], document_id="journal")

# Subsequent retains append instead of replacing
client.retain(bank_id, items=[{"content": "Day 2 notes..."}], document_id="journal", update_mode="append")
```
...
The `hindsight-hermes` integration package has been removed. Hermes Agent now ships with a [native Hindsight memory provider](https://github.com/NousResearch/hermes-agent/pull/5094) built into the framework itself, making the external integration package unnecessary.

## Other Updates

**Features**
- Added OpenCode persistent memory plugin for the OpenCode editor.
- Google embeddings and reranker provider support.
