...
LoComo and LongMemEval are still a valid foundation — the question formats are good, the evaluation methodology is reasonable, and they remain useful for catching regressions. But they're not enough. The next generation of memory benchmarks needs to test things that can't be solved by a big context window: multi-session accumulation at scale, cost-efficient recall, knowledge updates over time, and memory in agentic settings where the agent itself decides what's worth keeping.
...
**One-shot RAG**: a single query against the memory system, results passed directly to the LLM for answer generation. Fast, predictable, low latency. The tradeoff is coverage — one query may not surface everything needed for multi-hop questions where the answer requires connecting facts from different parts of the memory.

**Agentic RAG**: the LLM drives retrieval through tool calls, issuing multiple queries, inspecting results, and deciding when it has enough to answer. Consistently better on complex and multi-hop questions. The tradeoff is latency and cost — more round-trips, more tokens, more time.
