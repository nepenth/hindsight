...
:::tip LiteLLM Provider (AWS Bedrock, Azure, and more)
Set `HINDSIGHT_API_LLM_PROVIDER=litellm` to use any model supported by [LiteLLM](https://docs.litellm.ai/docs/providers), including **AWS Bedrock**, **Azure OpenAI**, **Together AI**, **Fireworks AI**, and many more. Model names use LiteLLM's provider prefix format (e.g., `bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0`). For providers like AWS Bedrock that use environment-based authentication (IAM roles, `AWS_ACCESS_KEY_ID`), no API key is required.
