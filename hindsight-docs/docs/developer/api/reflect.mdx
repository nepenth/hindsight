...
For applications that need to process responses programmatically, you can request structured output by providing a JSON Schema via `response_schema`. When provided, the response includes both the natural language text and a parsed `structured_output` field.
...
with Hindsight() as client:
    ...
    print(response.text)
    # "Based on Alice's extensive ML expertise at Google..."

    # Structured output - parse back into Pydantic model
